...
export default function HeatChart(props: any) {
  ...
  const [options, setOptions] = useState<any>({
    ...
    accessibility: {
      point: {
        descriptionFormatter: (point: { index: number; value: any; }) => {
  ...
  useEffect(() => {
    ...
    const yCategories = data.map((item: { name: any; }) => item.name).concat('<b>TOTAL</b>');
    const rowData = data.map((item: { courses: any[]; }) => {
    ...
    const sourceData = zip(...rowData)
      .map((columnAry: string | any[], index: any) => {
